...
func TestDiskSpaceCollect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			collector := NewDiskSpaceCollector(cl, tt.config)

			b, err := os.ReadFile(fixtures.CommonFixturesPath + "expected_diskspace_metrics.txt")
			assert.NoError(t, err)
...
func TestDiskSpaceCollect_FailureDoesntPanic(t *testing.T) {
	...
	assert.NoError(t, err)
	collector := NewDiskSpaceCollector(cl, config)
