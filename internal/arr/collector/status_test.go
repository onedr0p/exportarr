...
func TestStatusCollect(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NoError(t, err)
			collector := NewSystemStatusCollector(cl, tt.config)

			b, err := os.ReadFile(fixtures.CommonFixturesPath + "expected_status_metrics.txt")
			assert.NoError(t, err)
...
func TestStatusCollect_FailureDoesntPanic(t *testing.T) {
	...
	assert.NoError(t, err)
	collector := NewSystemStatusCollector(cl, config)
